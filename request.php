...
$query1="SELECT*FROM verifieduser WHERE email='$me'";
...
while($row1=$run1->fetch_assoc()){
    ...
    $code1=$row1['code'];
    if($row1['image']!='-'){
         $image1='uploads/'.$row1['image'];
    }
    else{
        $image1='lilly.jpg'; 
    }
    $f=$row1['verifiedfriend'];
    $r=$row1['friend'];
    $s=$row1['send'];
}
if(password_verify($_SESSION['password'],$psw1)){
echo '<!DOCTYPE html>
<html>
    <head>
    ...
    <body class="bg-warning">
        ...
        <div class="container">
          ...
          <div class="bg-success"><br>
            ...
            $friend=explode(",",$f);
            $send=explode(",",$s);
            $a=array_merge($req,$friend,$send);
            $i=0;
            foreach($a as $c){
            if($c==$id){
                $i++;}}
            if($i==0){echo '<form action=" " method="post">
            <input type="hidden" name="email" value='.$email.'>
            <input type="hidden" name="me" value='.$me.'>
            <button type="submit" name="send" class="btn btn-success">Request</button>
            </form>';}
            foreach($req as $j){
                if($j==$id){echo '<form action=" " method="post">
            <input type="hidden" name="email" value='.$email.'>
            <input type="hidden" name="me" value='.$me.'>
            ...
            foreach($send as $j){
                if($j==$id){echo'<span class="bg-danger" style="padding:1%;border:1px solid black"><i class="glyphicon glyphicon-friend"></i>Send</span><br><br>';
                    }
            }
    ...
    <body class="bg-warning">
        ...
        <div class="container">
          ...
          <div class="bg-success"><br>
            ...
            <a href='.$image.'><image src='.$image.' class="img-rounded" width="100px" height="100px"></a></blockquote></div>
            <div class="panel-body bg-info">';
            echo '<span class="bg-danger" style="padding:1%;border:1px solid black"><i class="glyphicon glyphicon-friend"></i>Send</span><br><br>';
           echo '<p style="background-color:#5bc0de;color:grey;font-family:Arial;text-align:left">
           <blockquote class="bg-success" style="text-align:left;color:grey">
            <i class="glyphicon glyphicon-envelope"></i>&nbsp'.$email.'<br>
            <i class="glyphicon glyphicon-home"></i>&nbsp'.$address.'<br>
...
echo $send2;
if($send2=="-"){
    $sql2="UPDATE verifieduser SET send='$id' WHERE email='$me'";
    ...
    $arr2[]=$id;
    $r2=implode(",",$arr2);
    $sql2="UPDATE verifieduser SET send='$r2' WHERE email='$me'";
    $result2=mysqli_query($conn,$sql2);  
}
if($friend=="-"){
    $sql="UPDATE verifieduser SET friend='$id2' WHERE email='$email'";
    ...
    $arr[]=$id2;
    $r=implode(",",$arr);
    $sql="UPDATE verifieduser SET friend='$r' WHERE email='$email'";
    $result=mysqli_query($conn,$sql);
}
}
